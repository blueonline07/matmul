...
Matrix Matrix::multiplyStrassen(const Matrix &A, const Matrix &B)
{
    ...
    int n = A_padded.rows(); // now square n x n
    ...
    if (n <= threshold)
    {
        C_padded = multiplyNaive(A_padded, B_padded);
    }
    else
    {
        int k = n / 2;
        ...
        // Combine quadrants into C_padded
    ...
    // Remove padding
    Matrix C = C_padded.submatrix(0, 0, A.rows(), B.cols());
    return C;
}