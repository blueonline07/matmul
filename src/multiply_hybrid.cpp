...
Matrix Matrix::multiplyHybrid(const Matrix &A, const Matrix &B, int size, int rank)
{
    if (!A.canMultiply(B))
    {
    ...
    MPI_Scatterv((rank == 0 ? A_data_ptr : nullptr), sendcounts.data(), senddispls.data(), MPI_DOUBLE,
    ...
    MPI_Bcast((rank == 0 ? const_cast<double *>(B_data_ptr) : B_buf.data()),
 ...
 * - Total parallelism = MPI_processes × OMP_threads
 */
Matrix Matrix::multiplyStrassenHybrid(const Matrix &A, const Matrix &B, int size, int rank)
{
    if (!A.canMultiply(B))
    {
        throw std::invalid_argument(Matrix::INCOMPATIBLE_DIMENSIONS_MSG);
